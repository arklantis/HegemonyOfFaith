...
- It's a Miracle revival (failed 6+ times): real cause of "believers vanish at center" = reveal slot used applyInlineBelieverFaceStyle on a plain div (blank). Switched to PROVEN `.card-believer` + `data-index` CSS sprite (same as graveyard picker). Rewrote `animateItsAMiracleReveal` as an onEnd-driven chain + safety-net timeout, guaranteed `addToStockWithId(type,id)`, `stagedRevivalCardIds` so newBelievers skips dup add.
- Divine Inspiration "discard flies twice" (esp. Prophet): race — suppression mark was set in the ajax callback, so if the divineInspiration notif arrived first both server + local flights ran. Fixed: mark `markActionDiscardFlightSuppressed` BEFORE ajaxAction; suppression window 10s→30s.
- Zombie Army graveyard picker: added a "Confirm" button (select + confirm commits directly); card click selects (highlight via data-grave-id) instead of select-and-close.
- STILL PENDING (animation polish; test the above first): Secret Alliance theatrical center-table exchange (secretAllianceSwap direct flight reportedly "jumps"); Have a Charity + Prophet position hitch when prediction layout activates; Martyrdom dead believers don't visibly fly to graveyard for non-attacker viewers (Conspiracy snatch-to-winner works).
- All JS/PHP validated. Uncommitted on `feature/single-player-ai`.
...
Latest update (2026-06-13) — single-target defense card animation (phase 6e, JS/CSS only):
- Witch Hunt (war 8) / Spread Rumors (war 9): the defender's card now flies from their seat onto the center attack card (`.center-defense-overlay`, offset top-right, face-up) and both fly to the discard pile together at resolution, instead of the defense card jumping to discard by itself.
- `notif_defensePlayed`: for war 8/9 with a `current_center_action_card`, routes to new `attachCenterAttackDefenseOverlay(...)` and DEFERS the discard push; `moveCurrentCenterActionToDiscard` now also flies the overlay to discard and records it (after the attack card). Overlay reference reset in `beginCenterActionDiscardTracking` so a new center card never inherits a stale overlay. Other non-AOE defenses (faith war/debate board, breaking faith) keep prior behavior.
- Faith War (2) / Faith Debate (7) defense: designer chose the low-risk option — keep the VS board loading as-is (no rendering-timing/resync change), and when a defense card is played the card flies from the defender's seat to the CENTER and covers the VS board (`.faith-war-defense-overlay`, centered over `.faith-war-main`), then the existing `combatBlocked` flow clears the board (with the overlay) afterwards. `notif_defensePlayed` war 2/7 → `attachFaithWarDefenseOverlay(...)` instead of the old generic `addCombatCommitToArena` wrap; discard push kept immediate (block path clears the board rather than routing through moveCurrentCenterActionToDiscard). No combat-flow/state changes; JS+CSS only.

Latest update (2026-06-16) — auto bot-fill on under-filled table (phase 6u, supersedes 6t):
- Final designer decision: the real game is 4 humans. ANY table with <4 humans is inherently a practice/test table and is ALWAYS auto-filled with practice-AI bots up to 4 (no option). 4+ humans = a normal game, never filled. Removed the practice-mode game option (101) and its gating entirely — fill is driven purely by human count. `practice_ai_player_mask` is built only from the created bot ids. Still shows BGA's "player count not coherent" warning (fake-player approach); accepted for practice/testing. Note WHY proper solo is a big rework (researched): BGA-sanctioned solo keeps the player table = real humans and represents opponents as non-player "Automa" entities driven by game state — this game's entire engine assumes participant = player-table row + player_id (sects, combat, turn order, multiactive, notifications, scoring), so true Automa = dual participant representation across the whole codebase. Deferred to a later milestone; current fake-player fill is the testing stand-in.

Latest update (2026-06-16) — bot-fill gated behind practice mode (phase 6t, superseded):
- Fix: the AI bot-fill was UNCONDITIONAL (any table with <4 humans got AI), so a non-practice 2-human game silently became 2 humans + 2 AI. Now `setupNewGame` reads option 101 early into `$practice_mode_on` and only bot-fills when practice mode is ON. Practice OFF = a normal table, never auto-padded with AI. The practice-mode mask (mark all non-first seats AI) is gated on the same `$practice_mode_on`, so practice ON works whether the other seats are bots (solo) or test accounts (Studio 4-account method).
- Known BGA limitation: `players` is global (now 1-8), so the lobby still lets someone manually pick a 2-3 human NON-practice game, which the 4-8-designed engine doesn't properly support. Mitigated by `suggest_player_number => 4`. Not guarded further (designer chose to rely on the lobby suggestion rather than add a hard block); revisit if needed.

Latest update (2026-06-16) — stray combat-believer deadlock fix (phase 6s):
- Bug: an edge-case Faith War final round left a committed Believer on `cardsontable` without resolving/clearing it; the next Conspiracy's commit guard (`playBelieverCard`, war_type 6, iterates the player's `cardsontable` Believers) then threw "You already committed a Believer for Conspiracy" and deadlocked. Reported during AI practice.
- Fix (PHP): new `returnStrayCombatBelieversToHands()` moves any Believers on `cardsontable` back to their owners' hands; called at the START of `playActionCardInternal` (action cards are only played on a single-active player's turn, never mid-combat, so any cardsontable Believer there is a stale leftover — safe to return). This breaks the "previous combat leftover → next combat deadlock" cascade.
- NOT fixed (needs reproduction detail): the ROOT Faith War final-round edge case where a Believer is not resolved/cleared when the war ends. Capture which side ran out first, the top instruction text, and the last log lines next time to pin it down.
- Solo bot-fill (phase 6r) confirmed playable in Studio (shows the "player count not coherent (4 vs 1)" warning but the game runs); acceptable for local single-player practice testing. Production solo-vs-bots remains constrained by BGA's lobby player count.

Latest update (2026-06-15) — solo play: AI bot-fill to 4 seats (phase 6r, Studio-test version):
- Goal: a human can start with <4 players; empty seats are filled with practice-AI bots up to the 4-seat minimum. (Research confirms BGA has NO native bot API — this is the standard custom "fake player" approach: real rows in the `player` table that everything reads from.)
- `gameinfos.inc.php`: `'players' => array(1,2,3,4,5,6,7,8)`, `'suggest_player_number' => 4`.
- `setupNewGame`: after the human INSERT, if humans < 4, INSERT (4 - humans) bot players. Bot `player_id` = max(real id)+i (unique within the table), `player_no` continues after humans, empty `player_canal` (no notification channel), name "AI n", color/sect from the remaining pools. Bot ids collected in `$bot_player_ids` and OR'd into `practice_ai_player_mask` (always AI). Deck sizing (`$player_count`) and the opening deal now iterate ALL seats (`$all_player_ids` from loadPlayersBasicInfos) not just humans; starting Skill draft already used getSortedPlayerIds so bots get their 2-of draft. AI auto-runs via the existing practice-AI step system (driven by the connected human's client; token-gated, not active-player-gated).
- KEY RISKS to verify in Studio (this is the unproven part): (1) does the modern `Bga\GameFramework\Table` accept manually-inserted player rows? (2) player_no/turn-order correctness with bot rows; (3) end-game scoring/stats with bot players; (4) reconnection. If the framework rejects fake players, we need its bot/solo API instead. Avatars are empty (cosmetic).
- Up-to-8 AI (choose table size) is a follow-up once bot-fill is proven; the current version always fills to 4.

Latest update (2026-06-15) — proper BGA entry-point for AI: practice-mode game option (phase 6q):
- Context: BGA has no native "play vs AI bot" lobby; the only native AI is zombie takeover. This project's practice AI is custom. The BGA-idiomatic place to choose "AI opponents" before a game is a GAME OPTION (gameoptions.json), chosen at table creation — not an in-game panel or F12.
- Added gameoptions.json option `101` "Practice mode (AI opponents)" (1=Off default, 2=On, tmdisplay "AI opponents"). In `setupNewGame`, when option 101 == 2, every seat except the first (lowest player id = table creator = the human) is marked into `practice_ai_player_mask` at init, so AI seats are active from game start (first active state action `stPracticeAiActivePlayer` kicks them; gamedatas already exposes `practice_ai_player_ids`, so the client panel + watchdog see them too). Seats still must exist — in Studio, fill them with test accounts and start.
- The in-game `#ai_control_panel` (phase 6p) was REMOVED at the designer's request — it doesn't belong in a production/live UI. The game option is the sole proper entry point for starting vs AI. `window.hofAi` console helper is kept (hidden dev tool, not visible to players) for mid-game toggling during testing.

Latest update (2026-06-15) — in-game AI control panel (phase 6p):
- Goal: let a human enable AI seats from the UI instead of the F12 `hofAi` console. Added `#ai_control_panel` at the top of `#play_area` and `renderAiControlPanel()`: one toggle button per OTHER seat (shows "name: AI ON/OFF", blue when on) plus "All AI" / "No AI" shortcuts. Buttons route to the existing `togglePracticeAiByConsoleTarget(pid)` / `setPracticeAiByConsoleTarget("others", true)` / `clearPracticeAiByConsole()` (same server actions as the console helper). Panel re-renders in `setup` and on the `practiceAiPlayersChanged` notif; hides itself when there are no other seats. New `.ai-control-panel` CSS. The `window.hofAi` console helper is kept as-is. (Always visible in alpha; gate behind a game option later if multi-human tables need it hidden.)

Latest update (2026-06-15) — revival animation rewritten to reliable direct flight (phase 6o):
- It's a Miracle "believers reach center and vanish with the action card" persisted across 4 staged-reveal attempts (the reveal-stack-then-refly choreography kept failing, esp. the observer fly-out). Replaced with a robust DIRECT animation in `notif_greatMercy`: revived believers fly graveyard → reviver (owner: own hand via `animateRevivedBelieversToHand` + `pendingRevivedFromGraveyard` so `newBelievers` does the no-fly stock add; observers: a staggered temp flight graveyard → the reviver's table anchor), and the revival action card holds `flyMs + (n-1)*stagger + min(900, revealHold)` then flies to discard. No reveal-beside-the-card step, but it is reliable and shows believers flying to the player + card to discard with no vanish/dead-window. `animateItsAMiracleStagedRevival` is now unused (left in place; can be deleted later).
- AOE death/snatch animations (Martyrdom dead → graveyard; Conspiracy snatched → attacker hand) reported as "jumping" (no flight). Code path is correct and multi-fallback (`animateAoeBelieversToTargets`: clone flight → temp flight → safeSlide; `ownerByCardId` maps dead→0/graveyard, conspiracy stolen→attacker_owner with forceCloneFlight). Most likely these were swallowed by the deadlock chaos (out-of-order notifs / stale DOM on transaction restart). RE-TEST after the phase-6m deadlock fix; if still jumping, investigate the wrap being re-rendered between `revealAoeBelievers` and the gated flight.

Latest update (2026-06-15) — AI waits for redistribute FX; believer-count desync (phase 6n):
- Everyone is Equal / Chaos Coming overlap: after an AI used Everyone is Equal (believer redistribute + shuffle), the NEXT AI's turn started ~550ms later (the `playerTurn` step delay) while the ~4.5s shuffle/deal animation was still running, so the next AI's Divine Inspiration drew a flood of believers on top of the still-flying redistribute — and the on-table believer count drifted from the actual hand. Fix: `notif_practiceAiStepRequested` now also waits on `getRedistributeFxPendingDelayMs("believer"|"action")` (the Everyone is Equal / Chaos Coming FX-pending timestamps) for ALL states, so no AI step fires until the redistribute animation has finished (+300ms). This was the most likely cause of the right-panel believer count not matching the player's actual hand under AI play (animation race adding/removing the wrong cards). Re-test the count after this; if it still drifts, add an explicit current-player stock reconcile in `notif_publicCountsSync` (server count is authoritative and should equal the hand stock).

Latest update (2026-06-15) — practice-AI concurrency / deadlock + bot session guard (phase 6m):
- ROOT CAUSE of the AI playtest errors (`mysql_deadlock_restart_transaction` loop, duplicate Prophet guesses, "It is not your turn", table not clearing): the practice-AI watchdog (`kickPracticeAi`, 6s) re-issued a step request while one was already pending, so two `runPracticeAiStep` calls executed concurrently → DB deadlock → BGA restarts the transaction → actions re-run. Fixes:
  - `kickPracticeAi` now no-ops when `practice_ai_request_token != 0` (a step is already scheduled/in-flight); it only re-drives the AI when the chain has gone idle.
  - `runPracticeAiStep` consumes the token IMMEDIATELY after the match check (was after the state validation), closing the window where a duplicate/stale same-token request could run the AI turn a second time.
- AI surrender "It is not your turn": practice-AI actions arrive as an AJAX from the human running the AI, so `getCurrentPlayerId()` is that human, not the AI seat — `requireCurrentOrBotForActiveState` threw. Added a nesting-safe `bot_automation_depth` counter (incremented around `runBotAutomationTurn` via a new `runBotAutomationTurnInner`); while >0 the guard treats the call as a bot action for the expected player (switches active player, returns expected) instead of throwing. Zombie was unaffected (system context, getCurrentPlayerId==0) but the flag is harmless there.
- Note: the "table not cleared between turns / previous cards remain" and the early Faith War end were most likely downstream of the deadlock corrupting mid-transaction state; re-test after this fix. The Faith War depletion guard counts hand Believers, which is correct (survivors are parked in `warused`, losers in graveyard — both already fought), and matches the pre-existing `stChooseWarRepresentative` check.

Latest update (2026-06-15) — revival reveal robustness + turn-draw serialization (phase 6l):
- It's a Miracle staged reveal made robust: the "3 graveyard cards fly beside the revival card" step previously depended on each flight's `onEnd` to un-hide its reveal slot; if a flight returned early the slot stayed hidden and the reveal step vanished. Added a fixed-timer failsafe (`i*stagger + flyMs + 40`) that un-hides each slot regardless. Sequence is now reliably: revival card to center → 3 believers fly graveyard→beside it → ~1.2s hold → believers fly to player + card to discard (offset `flyMs*0.6`).
- AI pacing overlap: the turn-start Action refill (`drawActionCards`) was not a synchronous notification, so a new player's refill (and any AI play) could animate at the same time as the previous turn's believer/action draw, flying to different seats at once. Registered `drawActionCards` as synchronous (`flyMs + stagger*4`) so turn draws run in sequence — previous animation finishes, turn ends, then the next player/AI draws.
- Faith Debate 5-round cap confirmed intact: `stResolveFaithDebateDuel` already ends on `round >= 5` OR either Sect's hand Believers == 0 (committed believers are parked in `debateused` and returned at debate end, so "hand empty" correctly means "that side has fought all its believers"). The phase-6k `stFaithDebateDuel` guard is a compatible round-start safety net (reassign a dry rep, else end), NOT a war-style fight-to-empty change.

Latest update (2026-06-15) — combat defense/depletion fixes from AI playtest (phase 6k):
- Faith Debate could deadlock the same way Faith War did: `stFaithDebateDuel` had no depletion guard, so a representative whose Believers were all snatched (mental combat) was still asked to "choose a Believer". Fix: before the rep-missing check, reassign a dry rep to a combat-ready Sect member (`getSectCombatReadyPlayerIds`); if none, the rep becomes 0 and the existing `finalizeFaithDebate` ends the debate (which also `notifyPublicCountsSync` — corrects the stale right-panel Believer count seen during the stuck debate).
- Faith War / Faith Debate: after a Follower blocked, the Leader (and any other defending-Sect member holding a defense card) was still prompted "Play a matching defense / Skip". A single Great Mercy / Firm Faith blocks the whole 1v1 Sect attack, so once `war_attack_blocked` is set, every other active defender is auto-finished. Added to BOTH `playDefenseCard` (human) and `zombiePlayDefenseIfAvailable` (bot/AI) for war_type 2/7.
- Faith War / Faith Debate defended: cards vanished and never reached the discard pile at AI speed. Made the staged exit robust: `isDuelBoardDefense` no longer requires the board node; `attachFaithWarDefenseOverlay` always sets `pendingFaithWarDefenseOverlay` (visual only when the board exists); `moveDuelActionCardToDiscard` now pushes the tracked war/debate action card to the discard data even when the board slot/DOM card is absent. `flyFaithWarDefenseToDiscardThenClear` flies defense + action card to discard then clears the board.

Latest update (2026-06-14) — war deadlock + 3 animation fixes from AI playtest (phase 6j):
- CRITICAL: Faith War to depletion could deadlock. `stChooseWarRepresentative` (state 69) checked for a Sect running out of Believers, but the per-round duel state `stFaithWarDuel` (state 70) did NOT for regular war (only war_type 10 final struggle). When a side hit 0 mid-loop the table re-entered faithWarDuel and asked a 0-Believer player to "choose a Believer" → stuck. Fix: extracted `endFaithWarForDepletedSect(...)` (DRY, used by state 69's existing branch and the new guard), added a depletion guard at the top of `stFaithWarDuel` (war_type 12 → finalizeFaithWar, else the helper), and added the `endWar` => 31 transition to the faithWarDuel state so it can end there.
- It's a Miracle revival looked like "believers crash into the card, both vanish": the reveal stack was `dojo.place(..., centerCard, "after")` which could nest it inside the card wrap (overlap). Now appended to `central_arena` directly (flex sibling beside the card); hold capped at 1200ms; center-card discard offset to `flyMs*0.6` after the believers fly out so they don't vanish together.
- War → next recruit: a fast AI recruit right after a war showed the recruit card below/beside the still-clearing VS board. `showCenterActionCard` now removes any lingering `faith_war_board` / `.faith-war-banner` (and cancels their delayed clear timers) before placing the new center card.
- AOE commit prompt not switching to "waiting": after committing, `handleAoeBelieverCommitted` re-rendered buttons before the commit reached the arena, and the wrapper's `checkAction("playBelieverCard")` could still report true, flipping the prompt back to "commit". Added an `aoeCommitDoneByMe` latch (set on own commit, checked in `canCurrentPlayerCommitAoeBeliever`, reset at AOE round start in `syncAoeDefendersChooseState`).

Latest update (2026-06-14) — self believer draw flies to own hand (phase 6i):
- Bug: when the viewer was the drawing player, drawn Believers flew to their name/table anchor (the "right-side player name") instead of their hand at the bottom — felt wrong for your own draw.
- Fix (JS only): `animateDeckDrawToPlayer` now targets the viewer's own hand (`mybelievercards` / `myactioncards`) with `toScale: 1` when `pid === this.player_id`; other viewers still see the public deck → that player's table/name gain. Deck-source suppression on the actual `newBelievers` stock add is unchanged, so no double flight. Applies to all viewports including mobile by design — the downward motion toward the hand reads as "toward me" even if the hand is below the fold, so no viewport special-case is needed.

Latest update (2026-06-14) — single-target defense block hold (phase 6h):
- Bug: when a single-target attack (Witch Hunt 8 / Spread Rumors 9) was defended, the defense card flew onto the center attack card and both were whisked to the discard pile in the same beat ("playing darts") — no pause to read the block. Cause: `notif_combatBlocked` cleared the arena via `clearTransientArenaAfterAction(getUnifiedQuickClearDelayMs())` (~700ms), about the same time the overlay flight landed.
- Fix (JS only): when `this.pendingCenterDefenseOverlay` is set (a single-target defense overlay is in play), `notif_combatBlocked` now uses `getUnifiedCardFlyMs() + 1000` so the overlapped defense card holds ~1s on the attack card before both fly to discard together (`moveCurrentCenterActionToDiscard` already flies overlay + attack card with a small stagger).
- Faith War / Faith Debate defense now matches: designer wants defense → cover the VS board → ~1s hold → clear board → fly to discard (board stays loaded as before, only the exit is staged). New `pendingFaithWarDefenseOverlay` flag (set in `attachFaithWarDefenseOverlay`); `notif_combatBlocked` holds ~1s then runs `flyFaithWarDefenseToDiscardThenClear()` which flies the defense overlay + the war/debate action card (`moveDuelActionCardToDiscard`) to the discard pile, then clears the board. The discard-pile push for the faith-war defense card is now deferred to this staged exit (was pushed immediately in `notif_defensePlayed`).

Latest update (2026-06-13) — AOE defense visual sync + post-defense prompt (phase 6g):
- Bug A: in the AOE commit phase the defense card was functionally clickable (server accepted it) but still rendered grayed/soft-disabled. Cause: in `refreshActionCardReadinessVisuals` the `applyDefenseFocusDimming` block dimmed only non-defense cards and returned, but the matching defense card fell through to the later `if (believerSelectionPhase)` rule (true during AOE commit) which grayed it. Fix: the defense-focus block now always returns — dims every Action card except the matching defense card, then stops so no later rule (believer-commit dimming) can gray the defense card.
- Bug B: after playing the AOE defense, the top instruction still showed the "commit a Believer or defend" prompt (and the Confirm button lingered) instead of switching to waiting. Cause: re-renders re-evaluated `canCurrentPlayerCommitAoeBeliever`, which still returned true via the stale `isCurrentPlayerInAoeCommitTargets` args. Fix: `canCurrentPlayerCommitAoeBeliever` now returns false when `hasCurrentPlayerSectDefendedInAoe()` (set from the public concealed defensePlayed's sect_id), so every re-render shows the waiting state. `notif_defenseCommittedPrivate` now also drops the believer highlight, sets selection mode 0, clears the action buttons, and re-renders so the switch is immediate. JS only.

Latest update (2026-06-13) — AOE defense unclickable fix + It's a Miracle staged animation (phase 6f):
- AOE defense regression (single-member sect, holding the matching defense card, still couldn't click it): root cause was the client gate `currentPlayerHoldsAoeDefenseCard()` starting with `if (!checkAction("playDefenseCard", true)) return false;` — the migrated BGA wrapper's checkAction lags in multiactive states (same class of issue noted for secret alliance). Fix: gate now uses a robust OR — `checkAction || isCurrentPlayerInAoeCommitTargets() || isCurrentPlayerActiveAoeAssignmentLeader()` (new helper reads `active_leader_ids` from state args), plus a `hasCurrentPlayerSectDefendedInAoe()` guard. `canSelectActionCardsInState` and `applyDefenseFocusDimming` for the 4 AOE states now route through `currentPlayerHoldsAoeDefenseCard()` instead of `isCurrentPlayerActive() && checkAction(...)`. The action-selection-changed submit handler also accepts the robust holder check so a click submits even when checkAction lags. Server `playDefenseCard` was already state-aware (uses `$aoe_phase_exits[$state_name]` for the transition), so no stall there. JS only.
- It's a Miracle (revival) staged animation: removed the ~1.6s idle gap where the action card sat in center after believers already flew to hand. New `animateItsAMiracleStagedRevival(cards, ownerId)`: revived believers fly graveyard → a face-up overlapping reveal stack beside the center action card (revived types are public), hold for revealHold, then the owner's cards fly reveal-slot → hand via `addToStockWithId(type,id,slotId)` (observers fly to the owner's table anchor) at the same instant the action card flies to discard. `notif_newBelievers` skips the duplicate stock add for `stagedRevivalCardIds`; falls back to the old simple flight if the center card node is missing. New CSS `.center-reveal-stack/.center-reveal-card/.center-reveal-count`.
- Divine Inspiration + Have a Charity dead-window fix (low-risk timing only, NOT the center-reveal restructure): designer chose to keep the existing discard-fly + deck-draw-fly animations and just remove the idle hold. In `notif_divineInspiration` / `notif_haveACharity` (non-prophet branch only), the center action card is now scheduled to discard at `max(drawAnimMs, flowLeadInMs) + stagger*3` (right after the Believer draw flight lands) instead of `flowLeadInMs + getUnifiedPostDrawDiscardDelayMs(...)` which carried a ~1600ms centerHold. Sequence now reads: discarded Action cards fly to discard → Believers fly to player → center card flies to discard right after. `applyBelieverDeckDrawVisualSync`'s return already folds in the discard-fly start delay, so no extra lead needed. Prophet flow is untouched (still guarded by `if (!prophetFlow)`; its center discard happens on prophet resolve). `getUnifiedPostDrawDiscardDelayMs` is still used for the `drawActionSyncMs` notif-queue sync, so pacing of following notifications is unchanged.
- The fuller DI staged reveal (discard/believer overlap stacks with count badges) remains available as a future option reusing `.center-reveal-stack`, but was not needed once the dead window was removed.
- Divine Inspiration + Prophet discard fly: previously `discardAnimMs = prophetFlow ? 0 : ...` suppressed the discarded-Action-card flight whenever a Prophet prediction was involved (cards teleported to the pile). Now the discard flight always runs, so the prophet sequence reads: discarded Action cards fly to discard → first drawn Believer runs the Prophet prediction (plus Gate of Truth secondary if any) → remaining drawn Believers fly to the player → the Divine Inspiration card flies to discard (already scheduled by `notif_prophetPredictionResolved` final phase). The prophet believer-draw path itself is untouched (`animate_draw` still 0 in prophet flow); only the independent Action-card discard animation was re-enabled.

Latest update (2026-06-13) — central arena fixed height (phase 6e):
- Bug: the center placement area shrank when empty and grew when a card was played, so it visibly jumped each play/discard cycle.
- Cause: `.central-arena` is content-box; empty min-height was 160px (+16 padding = 176px total) but a placed card uses `.center-action-wrap` min-height 202px (+16 = 218px) → ~42px jump.
- Fix (CSS only): `--arena-empty-min-height` now equals the center-action-wrap content height at each breakpoint so empty and single-card states match — root 202px; narrow (<=900px) and mobile (<=640px) use `calc(var(--combat-card-h) + 13px)` to track their wrap min-height. Taller prophet/combat states (230/390px) are unchanged and intentional.

Latest update (2026-06-13) — Secret Alliance exchange animation fix (phase 6d):
- Bug: Secret Alliance is a card EXCHANGE, but the offered card animated flying to the DISCARD pile and the received card just appeared underneath, looking like "discard the top of two stacked cards" rather than a swap.
- Root cause in `chooseSecretAllianceCardInternal`: it reused `actionCardsDiscarded` (→ `animateActionCardsToDiscard`, flies to discard) + `newActionCards` (→ card appears) for what is actually a hand-to-hand swap.
- Fix: new notifications. Private `secretAllianceSwap` per participant carries `given_card_id` + `received_card{id,type}` + `other_player_id`; JS `notif_secretAllianceSwap` flies the offered card from hand to the OTHER seat (clone, not discard) then removes it from stock, and adds the received card flying in face-up from the other seat (`addToStockWithId(..., otherAnchorId)`). Public `secretAllianceExchanged` now animates two face-down cards crossing between the two `playertable_<id>` anchors for OBSERVERS only (participants are skipped to avoid double motion via `animateSecretAllianceCrossFlight`). The center Secret Alliance action card still flies to discard as before. AI/zombie path uses the same internal, so it benefits automatically.

Latest update (2026-06-13) — AOE defense selectability regression fix (phase 6c):
- Bug: after the merged AOE defense flow, AOE defense became completely unplayable — a single-member-sect Leader attacked by Martyrdom/Conspiracy who held the matching defense card could not click it (action-card stock stayed in selection mode 0).
- Root cause in `syncActionSelectionModeToCurrentState`: commit states set `believerSelectionPhase = true` (rep must select a believer), and the action-card mode was `!believerSelectionPhase && canSelectActionCards ? 1 : 0` → always 0. The believer-commit and concealed-defense selections must COEXIST, but the logic treated them as mutually exclusive (the old confirmDefense path worked only because its believerSelectionPhase was false).
- Fix: dedicated branch for the four AOE states (`martyrdom/conspiracyChooseBelievers` + `...ChooseRepresentative`) sets action-card mode to 1 whenever `canSelectActionCardsInState` is true (already gated on isCurrentPlayerActive + playDefenseCard available). `refreshActionCardReadinessVisuals` extended so `applyDefenseFocusDimming` + `defenseKindForReadiness` also cover these states, keeping only the matching defense card visually enabled. JS-only change; click handler already routed action selection to playDefenseCard.
